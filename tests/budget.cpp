...
static const std::unordered_map<int64_t, currency> NEAR_CAP_BALANCE = {
	{FUND_GROCERIES,         currency{75000}},  // $750  — $250  of room before $1k  floor
	{FUND_LONGTERM_SPENDING, currency{950000}}, // $9500 — $500  of room before $10k floor
	{FUND_RENT,              currency{190000}}, // $1900 — $1100 of room before $3k  floor
};
...
static inline budget SingleFixedPhaseExpense(bool allow_overdraw) {
	budget budget;
	budget.overflow_fund = FUND_OVERFLOW;
	budget_phase<fixed_target> phase;
...
TEST(BudgetApply, SingleFixedPhaseExpense_NotEnough_NoOverdraw) {
	budget budget = SingleFixedPhaseExpense(false);
	transaction transaction;
	transaction.amount = currency{-30000}; // -$300
	auto allocations = budget.apply(transaction, NO_BALANCE);
	AssertAllocationsMatch(allocations, {
...
TEST(BudgetApply, SingleFixedPhaseExpense_NotEnough_WithOverdraw) {
	budget budget = SingleFixedPhaseExpense(true);
	transaction transaction;
	transaction.amount = currency{-30000}; // -$300
	auto allocations = budget.apply(transaction, NO_BALANCE);
	AssertAllocationsMatch(allocations, {
...
TEST(BudgetApply, SingleFixedPhaseExpense_MoreThanEnough) {
	budget budget = SingleFixedPhaseExpense(false);
	transaction transaction;
	transaction.amount = currency{-60000}; // -$600
	auto allocations = budget.apply(transaction, NO_BALANCE);
...
TEST(BudgetApply, SinglePercentagePhaseExpense_Under100) {
	budget budget;
	budget.overflow_fund = FUND_OVERFLOW;
	budget_phase<percentage_target> phase;
...
TEST(BudgetApply, SinglePercentagePhaseExpense_Over100_NoOverdraw) {
	budget budget;
	budget.overflow_fund = FUND_OVERFLOW;
	budget_phase<percentage_target> phase;
...
TEST(BudgetApply, SinglePercentagePhaseExpense_Over100_WithOverdraw) {
	budget budget;
	budget.overflow_fund = FUND_OVERFLOW;
	budget_phase<percentage_target> phase;
...
budget MultiPhaseExpense() {
...

	{
		...
		groceries_target.fund_id = FUND_GROCERIES;
		groceries_target.amount = currency{50000};  // $500
		groceries_target.cap = currency{0};         // floor: don't go below $0
		groceries_target.allow_overdraw = true;
		first_phase.targets.push_back(groceries_target);
...

	{
		...
		lts_target.fund_id = FUND_LONGTERM_SPENDING;
		lts_target.amount = percentage{2500}; // 25%
		lts_target.cap = currency{100000};    // floor: don't go below $1k
		lts_target.allow_overdraw = false;
		second_phase.targets.push_back(lts_target);
...

	{
		...
		rent_target.fund_id = FUND_RENT;
		rent_target.amount = currency{150000}; // $1500
		rent_target.cap = currency{0};         // floor: don't go below $0
		rent_target.allow_overdraw = true;
		third_phase.targets.push_back(rent_target);
...
static const std::unordered_map<int64_t, currency> FULL_BALANCE = {
	{FUND_GROCERIES,         currency{1000000}},
	{FUND_SAVINGS,           currency{1000000}},
	{FUND_LONGTERM_SPENDING, currency{1000000}},
	{FUND_RENT,              currency{1000000}},
};

TEST(BudgetApply, MultiPhaseExpense_Underallocated) {
	budget budget = MultiPhaseExpense();
	transaction transaction;
	transaction.amount = currency{-200000}; // -$2k
	auto allocations = budget.apply(transaction, FULL_BALANCE);
	AssertAllocationsMatch(allocations, {
		{FUND_GROCERIES,         currency{ -50000}},
		{FUND_SAVINGS,           currency{ -49995}},
		{FUND_LONGTERM_SPENDING, currency{ -37500}},
...
TEST(BudgetApply, MultiPhaseExpense_Overallocated) {
	budget budget = MultiPhaseExpense();
	transaction transaction;
	transaction.amount = currency{-500000}; // -$5k
	auto allocations = budget.apply(transaction, FULL_BALANCE);
...
static const std::unordered_map<int64_t, currency> NEAR_FLOOR_BALANCE = {
	{FUND_GROCERIES,         currency{25000}},  // $250  — $250  of room before $0  floor
	{FUND_LONGTERM_SPENDING, currency{150000}}, // $1500 — $500  of room before $1k floor
	{FUND_RENT,              currency{110000}}, // $1100 — $1100 of room before $0  floor
};

TEST(BudgetApply, MultiPhaseExpense_NearFloors) {
	budget budget = MultiPhaseExpense();
	transaction transaction;
	transaction.amount = currency{-500000}; // -$5k
	auto allocations = budget.apply(transaction, NEAR_FLOOR_BALANCE);
	AssertAllocationsMatch(allocations, {
		{FUND_GROCERIES,         currency{ -25000}},
		{FUND_SAVINGS,           currency{-158317}},
		{FUND_LONGTERM_SPENDING, currency{ -50000}},
