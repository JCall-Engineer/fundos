...
struct currency {
	...
	static std::optional<currency> from_string(const std::string& text, const currency_locale_info& locale) {
		auto parsed = parse_currency(text, locale);
		if (!parsed) { return std::nullopt; }
		return currency{*parsed};
	}
	std::string to_string(const currency_locale_info& locale) const { return format_currency(minor_units, locale); }
